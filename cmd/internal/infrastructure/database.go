...
// IDataBaseProvider is an interface to access a database
type IDataBaseProvider interface {
	...
	UpdateMany(collectionName string, filter interface{}, val interface{}) error

	UpdateOne(collectionName string, filter interface{}, val interface{}) error
	...
	Count(collectionName string, filter interface{}) (int64, error)
}
...
func (provider *MongoDataBaseProvider) ConnectDatabase() (interface{}, error) {
	onceDatabase.Do(func() {
		...
		uriDS := fmt.Sprintf("mongodb://%s:%d/%s", dbHost, dbPort, dbName)
		...
		if err != nil {
			logrus.Infof("couldn't reach database: %v", err)
			panic(err)
		}
		...
		if err != nil {
			logrus.Errorf("couldn't reach database: %v", err)
			panic(err)
		}
...
func (provider *MongoDataBaseProvider) Insert(collectionName string, val interface{}) (interface{}, error) {
	// get collection https://github.com/pedidosya/categorization-batchfrom database, if not exist, it is created,
	collection := provider.client.Collection(collectionName)
	// insert the interface in collection
	...
	logrus.Infof("inserted a single document: %s, in collection: %s", insertResult.InsertedID, collectionName)
...
func (provider *MongoDataBaseProvider) UpdateMany(collectionName string, filter interface{}, val interface{}) error {
	collection := provider.client.Collection(collectionName)
	updateResult, err := collection.UpdateMany(context.TODO(), filter, val)
	if err != nil {
		return fmt.Errorf("error to replace in collection: %s, %v", collectionName, err)
	}

	logrus.Infof("update %d documents in collection: %s", updateResult.ModifiedCount, collectionName)
	return nil
}

func (provider *MongoDataBaseProvider) UpdateOne(collectionName string, filter interface{}, val interface{}) error {
	collection := provider.client.Collection(collectionName)
...
func (provider *MongoDataBaseProvider) ReplaceById(collectionName string, id interface{}, val interface{}) error {
	...
	logrus.Infof("modified %d document: ", updateResult.ModifiedCount)
	return nil
}
...
func (provider *MongoDataBaseProvider) Count(collectionName string, filter interface{}) (int64, error) {
	collection := provider.client.Collection(collectionName)
	count, err := collection.CountDocuments(context.TODO(), filter)
	if err != nil {
		return 0, fmt.Errorf("error to find in collection: %s", collectionName)
	}
